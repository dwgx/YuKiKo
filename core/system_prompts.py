    ...
    def personality_system_prompt(
        *,
        ...
        constraints = _pl.get("personality_constraints",
            ...
            # 平台合规硬线。放在硬约束里而不是只写在人格底稿，是因为
            # 人格底稿那一行「政治敏感 = 回避」实测挡不住整段议论：
            # 输出侧的敏感词表只能替换单词，替换不掉一段不含表内词的政治评论，
            # 而这类内容是 QQ 群被封的主要成因之一。
            "- 时政红线 不谈中国政治 政党 领导人 政策评价 时政新闻 历史政治事件\n"
            "  被问到就自然转开话题 不要复述问题 不要解释为什么不谈 不要提审查或规则\n"
            "  转述搜索结果 网页内容 视频字幕时同样适用 别人的原话也不转\n"
            "- 成人红线 露骨性内容 R18 未成年相关一律不产出 也不描述 不引用 不转述\n"
            "  搜索或解析到这类素材时 不要把内容念出来 只说这个看不了")

...
## 0. 【唯一入口】2026-08-06 20:30 的真实状态

> 这一节是权威入口。文中另外几处「【下一个 AI 从这里开始】」标记都已过期：
> **§11 的六项待办已全部做完**（见 §12.8），**§13.2 的触发门也已做完**。
> 那些标记留着是为了保留当时的推理过程，**不要照它们的顺序动手**。
>
> 本文约 1776 行，§13 的小节编号顺序是乱的（13.6 → 13.8 → … → 13.11 → 13.16 → 13.7 → 13.3），
> 因为是多轮插入形成的。按标题找，不要按编号顺序读。

### 现在跑着什么

```
分支      refactor/prompt-driven-intent，工作区干净（0 未提交）
测试      1436 passed / 0 failed；scripts/project_takeover_selfcheck.py = PASS
机器人    在跑，单实例。确认方法：lsof -nP -iTCP:8081 -sTCP:LISTEN
NapCat    已连（ESTABLISHED=2）。诊断：bash scripts/napcat_macos.sh --status
白名单    [974118886]，non_whitelist_mode=silent（实测有效，15 次拦截、零误回）
```

### 唯一的高优阻塞：QQ 拒收富媒体

```
ActionFailed(retcode=1200) / EventChecker Failed: rich media transfer failed
```

它同时压着**表情包**和**语音**——两者的代码侧都已修好且验证过，但发不出去。
08-06 04:37 曾连拒 19 条并触发发送自我暂停。**这是 NapCat/QQ 侧问题，不是本仓代码。**
详见 §13.16(c)，那里列了三个可查方向。

### 代码侧还没做的（按价值排序）

1. **`search_knowledge` 无 scope 过滤 → 跨用户资料泄漏**（§13.6 表格第一行）。
   子 agent 审计报为 critical，我核过位置但没修。这是唯一的隐私类缺口。
2. `core/knowledge.py:496` FTS5 对中文返回 `[]`，LIKE 兜底是死代码 →
   **54 条线上知识条目里 53 条搜不到**（§13.6）。
3. `search_web_media` / `web_search(mode=image)` 跳过了 router 路径已有的成人内容
   黑名单（§13.6）。直接关联业主的 R18 顾虑。
4. `core/memory.py:372` `_connect` 按线程缓存 SQLite 连接但没按 db_path 区分 ——
   与 `knowledge.py` 已修的是同一个 bug（§13.6）。
5. 点歌匹配偏差：实测「周杰伦 稻香」命中的显示是「稻香 - 周杰伦-/Montagem」，
   像混音/串烧而非原版。能播，但匹配不对。

§13.6 还有 6 条我判为低优或已被后续改动覆盖的，看那张表。

### 动手前必读的两件事

**§10.0 那个问题**：「这个检查在 bug 完好无损的情况下会不会照样通过？」
我这几轮**自己犯了三次假绿灯**，全部记在 §13.12 / §13.13 / §13.16(d)：
`ast.unparse` 把双引号改成单引号让判据永不命中、
`registry.call` 吞异常让 `except NameError` 永不触发、
子串匹配源码时匹配到了自己写的注释。

**§13.16(d) 的操作坑**：`pgrep -f "[p]ython main.py"` 匹配不到（真实命令行是大写
`Python`），我因此连续几次「重启」是空操作、一段时间里以为在测新代码实际跑的是旧进程。
判断实例数要用 `lsof -nP -iTCP:8081 -sTCP:LISTEN`，不要用 `ps aux | grep -c`。

### 本轮新增的工具

```
bash scripts/napcat_macos.sh --status        NapCat 接线诊断（幂等，不动任何东西）
.venv/bin/python scripts/tool_smoke_live.py  对运行中的机器人跑端到端工具调用
                                             默认干跑，必须 --real-group 才真发
```

---

> 以下是历史记录，保留推理过程与证据。**§1（业主原话）、§4（踩过的坑）、
> §8（工作方式约定）仍然有效**，其余各节的「当前状态」类结论都已被 §0 取代。
...
## 11. ~~【下一个 AI 从这里开始】~~【已全部完成，见 §12.8】重启后的真实日志分析

> **这一节的六项待办已全部做完**（§12.8 有逐项状态）。而且其中三条结论被后续实测
> 推翻：`qq_vkey_no_purl` 是凭证墙不是 bug、migu 根因是参数名不是 301、
> 音源是并发批次不是串行（见 §12.0）。
> 保留原文是为了留住当时的证据与推理，**不要照这里的顺序动手**，看 §0。
...
### 13.2 ~~【下一个 AI 从这里继续】~~【已完成】触发门只做了一半

> **三步都做完了**（本节末尾有逐项结论）。标题保留原样是为了留住
> 「模板优先于 Python payload」那个教训。当前待办看 §0。
